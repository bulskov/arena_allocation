...
static void setup(void)    { cr_assert_eq(virtual_arena_init(&arena, RESERVED, COMMIT_CHUNK), 0); }
static void teardown(void) { virtual_arena_destroy(&arena); }
...
Test(virtual_arena, alloc_beyond_reserved_returns_null)
{
    ...
    cr_assert_eq(virtual_arena_init(&small, mem_page_size(), mem_page_size()), 0);
