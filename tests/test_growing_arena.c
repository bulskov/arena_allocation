...
static void setup(void)    { growing_arena_init(&arena, BLOCK_SIZE); }
static void teardown(void) { growing_arena_destroy(&arena); }
...
{
    allocator_t a = growing_arena_allocator(&arena);
    mem_alloc(a, BLOCK_SIZE, 1);     /* fills first block */
    mem_alloc(a, BLOCK_SIZE, 1);     /* forces second block */
